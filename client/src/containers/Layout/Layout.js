...
const Layout = ({ children }) => (
    <div className='relative duration-200 transition-color m-auto bg-background-secondary h-full text-center flex flex-col md:gap-0 md:grid md:grid-flow-row md:grid-cols-6 md:grid-rows-6 fhd:grid-cols-8 fhd:max-w-fhd'>
        ...
        <div className='absolute m-5 right-0 text-nav md:text-default-gray md:m-6'>
            <ThemeToggler />
        </div>
        <div className='overflow-y-auto max-h-screen px-6 flex-auto md:py-6 md:flex-none md:col-span-5 md:row-span-6 fhd:col-span-7'>
            {children}
        </div>
